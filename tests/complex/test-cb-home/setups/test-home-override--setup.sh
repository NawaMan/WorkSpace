...
# -----------------------------------------------------------------------------
# test-home-override--setup.sh
#
# Test setup script that:
# 1. Creates a file in /home/coder during build (BEFORE cb-home runs)
...
echo "Setting up test for cb-home..."

# Create the test file in /home/coder DURING BUILD
# This file SHOULD be overwritten by cb-home (override behavior)
mkdir -p /home/coder
...
# This allows testing that cb-home CAN copy files when they don't exist

# Create a startup script that logs the file content
cat > /usr/share/startup.d/70-cb-test-home-override--startup.sh << 'STARTUP'
#!/usr/bin/env bash
# Startup script to log the test file content for verification
echo "=== cb-home test startup ===" >> /tmp/startups.log
...
chmod 755 /usr/share/startup.d/70-cb-test-home-override--startup.sh

echo "Test setup for cb-home complete."
