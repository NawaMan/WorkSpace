...
VENV_DIR_BAKED="$VENV_DIR" \
envsubst '$VENV_DIR_BAKED' > /usr/local/bin/codeserver <<'LAUNCH'
#!/usr/bin/env bash
set -Eeuo pipefail

# Bake in the build-time value here only:
...
source /opt/venvs/py${PY_VERSION}/bin/activate
