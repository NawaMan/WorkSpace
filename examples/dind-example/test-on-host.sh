...
../../workspace --keep-alive --daemon > /dev/null 2>&1 || true
...
docker exec "$CONTAINER_NAME" bash -c "cd /home/coder/workspace && ./start-server.sh" > /dev/null 2>&1
pass "Server started"
...
docker exec "$CONTAINER_NAME" bash -c "cd /home/coder/workspace && ./stop-server.sh" > /dev/null 2>&1
pass "Server stopped"
