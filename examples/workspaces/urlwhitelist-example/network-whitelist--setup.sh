...
WHITELIST_DISABLE_CMD="/usr/local/bin/network-whitelist-disable"
WHITELIST_ENABLE_CMD="/usr/local/bin/network-whitelist-enable"
...
ENABLED_FLAG="$HOME/.network-whitelist-enabled"
PROXY_PORT=18888

# Check if whitelist is enabled (default: disabled for backwards compatibility)
if [ ! -f "$ENABLED_FLAG" ]; then
    echo "ℹ️  Network whitelist is disabled. Run 'network-whitelist-enable' to enable."
    exit 0
fi

echo "🔒 Setting up network whitelist proxy..."
...
sleep 1
if pgrep -x tinyproxy > /dev/null 2>&1; then
    echo "✅ Network whitelist proxy is running on port ${PROXY_PORT}"
else
    echo "⚠️  Warning: tinyproxy may not have started correctly"
fi
EOF
...
# Sets HTTP_PROXY/HTTPS_PROXY when whitelist is enabled
...
for domain in "$@"; do
    ...
    if grep -qxF "$domain" "$USER_WHITELIST" 2>/dev/null; then
        echo "ℹ️  '$domain' is already in whitelist"
    else
...
if [ ! -f "$ENABLED_FLAG" ]; then
    echo "⚠️  Network whitelist is disabled. Run 'network-whitelist-enable' first."
    exit 1
fi

echo "🔄 Reloading network whitelist..."
...
if pgrep -x tinyproxy > /dev/null 2>&1; then
    ...
    echo "✅ Whitelist reloaded and proxy signaled"
else
    echo "⚠️  Proxy not running. Starting it..."
    sudo tinyproxy -c /etc/tinyproxy/tinyproxy.conf
    echo "✅ Proxy started with new whitelist"
fi
EOF
...
if [ -f "$ENABLED_FLAG" ]; then
    ...
    echo ""
    echo "Run 'network-whitelist-enable' to enable network restrictions."
    exit 0
fi
...
# ---- Create utility: network-whitelist-enable ----
cat > "${WHITELIST_ENABLE_CMD}" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if [ -f "$ENABLED_FLAG" ]; then
    echo "ℹ️  Network whitelist is already enabled."
else
    touch "$ENABLED_FLAG"
    echo "✅ Network whitelist enabled."
    echo ""
    echo "Starting proxy..."
    # Trigger the startup script
    source /usr/share/startup.d/40-cb-network-whitelist--startup.sh
    echo ""
    echo "IMPORTANT: You need to reload your shell or run:"
    echo "  source /etc/profile.d/40-cb-network-whitelist--profile.sh"
    echo ""
    echo "Or simply start a new terminal session."
fi
EOF
chmod 755 "${WHITELIST_ENABLE_CMD}"

# ---- Create utility: network-whitelist-disable ----
cat > "${WHITELIST_DISABLE_CMD}" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if [ ! -f "$ENABLED_FLAG" ]; then
    echo "ℹ️  Network whitelist is already disabled."
else
    rm -f "$ENABLED_FLAG"

    # Stop tinyproxy if running
    if pgrep -x tinyproxy > /dev/null 2>&1; then
        sudo pkill tinyproxy || true
        echo "🛑 Stopped proxy."
    fi

    echo "✅ Network whitelist disabled."
    echo ""
    echo "IMPORTANT: The proxy environment variables are still set in this session."
    echo "Start a new terminal session or run:"
    echo "  unset HTTP_PROXY HTTPS_PROXY http_proxy https_proxy"
fi
EOF
chmod 755 "${WHITELIST_DISABLE_CMD}"

# ---- Allow coder user to manage tinyproxy via sudo without password ----
echo "🔐 Configuring sudo permissions for tinyproxy..."
...
echo ""
echo "✅ .... Network Whitelist is installed ...."
echo "• Startup file : ${STARTUP_FILE}"
echo "• Profile file : ${PROFILE_FILE}"
echo "• Default whitelist: ${DEFAULT_WHITELIST}"
echo "• User whitelist   : ~/.network-whitelist"
echo ""
echo "Commands:"
echo "  network-whitelist-enable   - Enable network restrictions"
echo "  network-whitelist-disable  - Disable network restrictions"
...
echo ""
echo "NOTE: Network whitelist is DISABLED by default."
echo "      Run 'network-whitelist-enable' to activate."
echo ""
