...
func TestPrintCmd_SimpleArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "docker run ubuntu:20.04 \n",
		},
		{
			name:     "args with allowed special chars",
			command:  "docker",
			args:     []string{"build", "-t", "my-image:1.0", "/path/to/dir"},
			expected: "docker build -t my-image:1.0 /path/to/dir \n",
		},
	}
...
func TestPrintCmd_RealWorldExamples(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: "docker build -f Dockerfile -t workspace-local:myproject-default-latest --build-arg 'WS_VARIANT_TAG=default' /workspace \n",
		},
		{
			...
			expected: "docker run -it --rm ubuntu bash -lc 'echo '\\''Hello World'\\''' \n",
		},
	}
...
func TestPrintCmd_OutputFormat(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "/usr/bin/docker /path/to/file \n",
		},
		{
		...
		},
		{
			...
			expected: "docker run 'my image' '--name=test' \n",
		},
	}
